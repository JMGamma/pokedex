...
func getLocationAreaData(url string, cfg *config) string {
	var body []byte
...
func getPokemonEncounters(url string, cfg *config) string {
	var body []byte
	if val, ok := cfg.cache.Get(url); ok {
		body = val
	} else {
	...
	for i := 0; i < len(data.PokemonEncounters); i++ {
		printout += fmt.Sprintf("%s\n", data.PokemonEncounters[i].Pokemon.Name)
	}
	return printout
}

func getPokemonCatchRate(url string, _ *config) int {
	var body []byte
	resp, err := http.Get(url)
	if err != nil {
		fmt.Printf("Error fetching data: %v\n", err)
